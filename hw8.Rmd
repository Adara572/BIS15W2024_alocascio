...
```{r}
#yes the data is tidy and in wide format but all of the cobject and variables are displayed proerly in the columns
```

#long format and is tidy
...
```{r}
#sydneybeaches_long %>% 
  #pivot_longer(separate, into = c("day", "month", "year"), names_sep = "/")
```


7. What is the average `enterococci_cfu_100ml` by year for each beach. Think about which data you will use- long or wide.
```{r}
#average_enterocci <- sydneybeaches_long %>% 
  #pivot_longer(separate, into = c("day", "month", "year"), names_sep = "/") %>% 
  #group_by(site, year) %>% 
...
```{r}
#select(average_enterocci_wide, "site","2013")
```
