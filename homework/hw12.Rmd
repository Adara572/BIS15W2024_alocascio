...
```{r}
wolves <- read.csv("../data/wolves_dataset.csv")
```
...
```{r}
ggmap(map2) +
  geom_point(data = wolves, aes(long, lat), size=1.5)+
...
```{r}
ggmap(map2) +
  geom_point(data = wolves, aes(long, lat, color=pop), size=1.5)+
  labs(x="Longitude", y="Latitude", title= "Wolves Locations")
```
