...
penguins %>% 
  group_by(species) %>% 
  summarize(mean_bill_lenght=mean(bill_length_mm, na.rm=T))
```
...
penguins %>% 
  group_by(species) %>% 
  summarise(across(c(contains("mm"),body_mass_g),mean, na.rm=T), # notice that I can include variables other than the ones that have mm
            n_samples=n())
```
