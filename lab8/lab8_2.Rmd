...
```{r}
life_history <- life_history <- read_csv("data/mammal_lifehistories_v3.csv",na= c("NA", " ", ".", "-999", "not measured")) %>% clean_names()
# you need to no how the NAs are represetned in the data
#you dont wanna do this by default
...
#makes replcement of NA's specific for variables
```
...
```{r}
#cites <- read.csv("data/cites.cvs") %>% clean_names()
```
...
```{r}
cites %>% 
  select(class, exporter_reported_quanity) %>% 
  group_by(class) %>% 
  miss_var_summary() %>% 
