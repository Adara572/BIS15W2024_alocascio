...
msleep %>% 
  ...
  summarise(mean_bodyweight=mean(bodywt),
...
```{r}
msleep %>% 
  summarise(n_genera=n_distinct(genus)) #this is ging to count the number of genera in msleep
```
```{r}
glimpse(msleep)
```
...
```{r}
msleep %>% 
  summarise(min_sleept=min(sleep_total),
            max_sleept=max(sleep_total),
            mean_sleept=mean(sleep_total),
            total=n())
```
